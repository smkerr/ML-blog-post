...
Misinformation has always been a threat to society and democracy. Today, this danger is fueled by the potential of its rapid diffusion through social media. Machine learning (ML) and Natural Language Processing (NLP) enable us to detect fake news so that we might warn the reader and raise awareness about misinformation. This blog post will therefore present our work on the identification of such so-called “fake news” on social media platforms.
...
1. Building a fake news detection algorithm with a high level of effectiveness using the [Covid-19 Fake News dataset](https://paperswithcode.com/dataset/covid-19-fake-news-dataset). This part is based on similar work performed by *Dipta Das et al.* who used the same dataset to create fake news detection models [@dipta2021heuristic].
2. Applying our fake news detection algorithm to more recent data which we have collected ourselves by replicating the methods of the authors of the original dataset. In this stage, we address the aforementioned issue of the time- and context-related decline in model performance. 

This blog post will present our approach to Covid Fake News Classification by

* providing a brief overview of related research
* explaining our method (pre-processing, training and testing of model on new data)
* presenting our experiments and results
